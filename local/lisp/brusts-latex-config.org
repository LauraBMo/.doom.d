...
(defun brust-list--add-colon (element)
  (cons (car element) (concat (cdr element) ":")))
(defun brust-list--add-colons (list)
  (mapcar #'brust-list--add-colon list))

;; (brust-list--add-colons brust-LaTeX-environments)
#+END_SRC
...
#+begin_src elisp
  (defun brust-LaTeX-set-header (-header)
    (interactive
     (list (ivy-read "Header: " (mapcar #'car brust-LaTeX-headers))))
    (let ((title (read-string (format "%s's title: " (capitalize -header)))))
      (if (string= title "")
          (message "No title, no %s" (capitalize -header))
        (let ((label (read-string (format "%s's label: " -header)
...
#+end_src
** Insert change to math mode
#+begin_src elisp
(advice-add 'TeX-insert-dollar :before #'brust-evil-insert--to-normal-to-insert-state)
(advice-add 'brust-LaTeX-insert-math2 :before #'brust-evil-insert--to-normal-to-insert-state)
#+end_src
...
#+BEGIN_SRC elisp
  ...
  (setq LaTeX-section-label
        `,(brust-list--add-colons brust-LaTeX-headers))
#+END_SRC
...
#+BEGIN_SRC elisp
  ...
  (add-to-list 'TeX-command-list
               '("LuaLaTeX-mk" "latexmk -pdf -pvc -pdflatex=\"lualatex --shell-escape -interaction=nonstopmode -file-line-error --synctex=1\"  %s"
                 TeX-run-TeX nil t
                 :help "Run Latexmk (with LuaLaTeX) on file to build everything.")
               t)
#+END_SRC
  ...
  (defvar brust-cdlatex-text-mode-snippets
    (append
     ...
     '(;; ("ci" "Insert citation" "" org-ref-helm-insert-cite-link nil t nil)
       ...
       ;; ("pg"         "Make a paragraph"                "\\footnote{?}"         cdlatex-position-cursor nil t   nil)
...
** COMMENT Adaptation to my style
